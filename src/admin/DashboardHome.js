        ...
        {STAT_CARDS.map((card, i) => {
          ...
          return (
            <motion.div key={card.key} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              className={`glass rounded-2xl p-5 shadow-lg ${card.glow} cursor-default`}>
              <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${card.from} ${card.to} flex items-center justify-center mb-4 shadow-lg`}>
                <Icon size={18} className="text-white" />
              </div>
              <div className="text-3xl font-bold text-white tabular-nums">{stats[card.key]}</div>
